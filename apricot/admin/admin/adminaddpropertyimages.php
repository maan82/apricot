...
sessionStart("Add or Edit Property");
 ...
 require('imageresize_phmagick.php'); 

 require( 'phmagik/phmagick.php'); 
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_adminaddprojectimages")) 
{
	...
	foreach ($_FILES["userfile"]["error"] as $key => $error) 
	{
			...
			{
				if ($_FILES['userfile']['size'][$key] > $MAX_FILE_SIZE) {
			...
			if ($error == UPLOAD_ERR_OK && !$errorMsg[$_FILES['userfile']['name'][$key]]) 
			{
			   ...
			   $image->resizeToFullIM($uploadfile, $fullpath);
			   
			   
/*
 * OLD CODE TO RESIZE IMAGE USING GD
 * 
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_adminaddprojectimagesFP")) 
{
	...
	foreach ($_FILES["userfile"]["error"] as $key => $error) 
	{
			...
			if ($error == UPLOAD_ERR_OK && !$$errorMsg[$_FILES['userfile']['name'][$key]]) 
			{
			   ...
			   $fullpath = $propdir.'/'.$picture_ID.'_FULL_'.$uid .$fileext;
			   $image->resizeToFloorPlanIM($uploadfile, $fullpath);
			   $width = $image->getWidthIM($fullpath);
			   $height = $image->getHeightIM($fullpath);

   			   $widthorg = $image->getWidthIM($uploadfile);
			   $heightorg = $image->getHeightIM($uploadfile);
			   

/**
 * OLD CODE TO RESIZE USING GD
 *
			   $image = new SimpleImage();
			   $image->load($uploadfile);
			   $thumbpath = $propdir.'/'.$picture_ID.'_THUMB_'.$uid .$fileext;
			   ...
			   $image = new SimpleImage();
			   $image->load($uploadfile);
   			   $widthorg = $image->getWidth();
			   $heightorg = $image->getHeight();
			   $fullpath = $propdir.'/'.$picture_ID.'_FULL_'.$uid .$fileext;
			   $image->resizeToFllorPlan();
...
*/			   
			   ...
			   $insertSQL = sprintf("INSERT INTO pictures ( PICTURE_ID, PROPERTY_ID, IS_MAIN, TITLE, COMMENTS, THUMB_PIC_PATH, SLIDE_PIC_PATH, FULL_PIC_PATH,FULL_WIDTH,FULL_HEIGHT,    ORIGINAL_PIC_PATH,ORIGINAL_WIDTH,ORIGINAL_HEIGHT) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
			   GetSQLValueString($picture_ID, "double"),																												               
			   GetSQLValueString($_POST['PROPERTY_ID'], "int"),
			   GetSQLValueString($mainimg, "text"),
			   ...
			   GetSQLValueString($uploadfile, "text"),
   			   GetSQLValueString($widthorg, "int"),
			   GetSQLValueString($heightorg, "int"));
...
<input type="button" name="cancel" class="button" value="Preview" onclick="gotoUrl('../pages/property-details.php?PROPERTY_ID=<?php if(isset($_GET['PROPERTY_ID'])){ echo $_GET['PROPERTY_ID'];}else if (isset($_POST['PROPERTY_ID'])){ echo $_POST['PROPERTY_ID'];} else { echo("-1") ;} ?>')"/>
