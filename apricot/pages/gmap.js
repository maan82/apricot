...
function gotoProperty(obj,lat,lon,indx,desc){
		...
		map.panTo(newPoint);
				...
				timerVisitAll2 = setTimeout(function(){markerArr[indx].openInfoWindowHtml(desc)},900)
	...
	localSearch.setSearchCompleteCallback(null, 
		function() {
			...
			{		
				...
				var resultLat = 51.4556;
				var resultLng = -0.409126;
				var point = new GLatLng(resultLat,resultLng);
				
				callbackFunction(point, "TW14 0JG");
...
function placeMarkerAtPoint(point, postcode)
{
	...
	icon.image = "images/apricot-map.png";
    ...
    marker.openInfoWindowHtml("<b>Please set property location using this marker.<br/>This is very important because initially marker may not be<br/> pointing to exact location of property.Also <br/>Street View location is based on this maker.Second<br/> marker is for address completion only.</b>");
  });
...
function numberOfPropShow(statusSpanID){
		...
		if(statusSpanID != null && document.getElementById(statusSpanID) != null)
 ...
 function addMarkerWithDetail(map, pointElement, openMarker, markerIndex) {
      if (GBrowserIsCompatible()) {
		var point = new GLatLng(pointElement.lat,pointElement.lon);
			...
			icon.image = "images/apricot-map.png";
			//icon.shadow = "http://www.google.com/mapfiles/shadow50.png";
			icon.iconSize = new GSize(124, 79);
			icon.shadowSize = new GSize(0, 0);
			icon.iconAnchor = new GPoint(43, 79);
...
function visitAllPlayPause(imgId,propArr){
	if(visitAllStatus == "P"){
...
function visitAll(propArr){
	clearTimeout ( timerVisitAll1 );
	clearTimeout ( timerVisitAll2 );
	clearTimeout ( timerVisitAll3 );
	if(visitAllStatus == "P")
	{
		timerVisitAll3 = setTimeout(function(){visitAll(propArr); propArr = null;},visitAllSpeed);
		return;
	}
	...
	if(nextNearProp == null){
			for(i=0;i<propArr.length;i++){
				propArr[i].visited = false;
			}
			presentPropCounter = presentProperty;
			timerVisitAll1  = setTimeout(function(){visitAll(propArr); propArr = null;},visitAllSpeed);
			return;
	} else {
		...
		timerVisitAll2 = setTimeout(function(){markerArr[nextNearProp].openInfoWindowHtml(propArr[nextNearProp].desc)},900)
...
function visitNextProperty(propArr,loop){
	propDetailsUpadted = false; //this variable controls start of flying
	var presentProperty = presentPropCounter;
	var nextNearProp = nextNearestProperty(presentProperty, propArr);
	if(nextNearProp == null){
			alert("You have visited all the properties.You can click View Next button to start over");
				for(i=0;i<propArr.length;i++){
					propArr[i].visited = false;
				}
	...
	movedToNextProp = true;
	$(".items img").removeClass("active");
	imgId = "scrollerImageId"+propArr[nextNearProp].id;
