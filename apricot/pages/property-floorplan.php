...
if (isset($_GET['PICTURE_ID']) ) {
  $colname_picturesID = $_GET['PICTURE_ID'];
	$query_fp_Img_RS = sprintf("SELECT * FROM pictures WHERE PROPERTY_ID = %s and TITLE IN('FLOORPLAN') and PICTURE_ID = %s ORDER BY COMMENTS DESC",GetSQLValueString($colname_picturesRS, "int"),GetSQLValueString($colname_picturesID, "int"));   
...
$query_fp_RS = sprintf("SELECT * FROM pictures WHERE PROPERTY_ID = %s and TITLE IN('FLOORPLAN') ORDER BY COMMENTS ASC", GetSQLValueString($colname_picturesRS, "int"));
...
<div >
 <div class="box_title">Floorplan </div>
 <div style="float:right">
 <form id="floorformId" method="get" action="property-floorplan.php" >
 <input name="PROPERTY_ID" type="hidden" value="<?php echo $_GET['PROPERTY_ID'] ?>" />
<span class="label">Floor :- </span> <select name="PICTURE_ID" onchange="document.getElementById('floorformId').submit()">
   <?php
do {  
?>
...
<div id="wrapp" > 
           ...
           <?php if (isset($_GET['PICTURE_ID']) ) { ?>
           	<img src="<?php echo $row_fp_Img_RS['ORIGINAL_PIC_PATH']; ?>"  title="Floor Number : <?php echo $row_fp_Img_RS['COMMENTS']; ?>" /> 
           <?php } else { ?>
				<img src="<?php echo $row_fp_RS['ORIGINAL_PIC_PATH']; ?>"  title="Floor Number : <?php echo $row_fp_RS['COMMENTS']; ?>" /> 
                <?php } ?>
</div> 
 			<br style="clear:both" /> 
...
<?php } else {?>
    <div class="label">
		<img src="images/info.png" alt="Information" width="32" height="32" align="absmiddle"> Floorplan not uploaded for this property.
<div class="clear"></div>
    </div>
